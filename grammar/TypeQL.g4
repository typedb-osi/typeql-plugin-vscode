...
// TypeQL end-of-file (aka. end-of-string) query parser Needed by TypeQL's Parser to ensure that it
// parses till end of string
...
query_insert:
	MATCH patterns INSERT variable_things
	| INSERT variable_things;
query_delete_or_update:
	MATCH patterns DELETE variable_things (
		INSERT variable_things
	)?;
...
// GET AGGREGATE QUERY =========================================================
// 
// An aggregate function is composed of 2 things: The aggregate method name, followed by the
// variable to apply the function to

match_aggregate:
	aggregate_method VAR_? ';'; // method and, optionally, a variable
aggregate_method:
	COUNT
	| MAX
	| MEAN
	| MEDIAN // calculate statistical values
	| MIN
	| STD
	| SUM;
...
// GET GROUP QUERY =============================================================
// 
// An group function is composed of 2 things: The 'GROUP' method name, followed by the variable to
...
variable_thing_any:
	...
	| variable_attribute;
variable_thing:
	VAR_ ISA_ type (',' attributes)?
	| VAR_ IID IID_ ( ',' attributes)?
	| VAR_ attributes;
variable_relation:
	VAR_? relation ISA_ type (',' attributes)?
	| VAR_? relation attributes?;
variable_attribute:
	VAR_? predicate ISA_ type (',' attributes)?
	| VAR_? predicate attributes?;

// RELATION CONSTRUCT ==========================================================

relation:
	'(' role_player (',' role_player)* ')'; // A list of role players in a Relations
role_player:
	type ':' player // The Role type and and player variable
...
attribute:
	HAS label (
		VAR_
		| predicate
	) // ownership by labeled variable or value
...
// COMPUTE QUERY ===============================================================
// 
// A compute query is composed of 3 things: The "compute" keyword followed by a method and
// optionally a set of input

compute_conditions:
	conditions_count ';' // compute the number of concepts
...
compute_method:
	MIN
	| MAX
	| MEDIAN // statistical value methods
	| MEAN
	| STD
	| SUM;
...
conditions_central:
	CENTRALITY input_central (',' input_central)*;
...
compute_direction:
	FROM IID_ // an instance to start the compute from
...
compute_algorithm:
	DEGREE
	| K_CORE
	| CONNECTED_COMPONENT; // algorithm to determine how to compute
compute_args:
	compute_arg
	| compute_args_array; // single argument or array of arguments
compute_args_array:
	'[' compute_arg (',' compute_arg)* ']'; // an array of arguments
compute_arg:
	MIN_K '=' LONG_ // a single argument for min-k=LONG
...
value: STRING_ | LONG_ | DOUBLE_ | BOOLEAN_ | DATE_ | DATETIME_;
regex: STRING_;

// UNRESERVED KEYWORDS ========================================================= Most of TypeQL
// syntax should not be reserved from being used as identifiers
...
// TYPEQL INPUT TOKEN PATTERNS All token names must end with an underscore ('_')
...
fragment TIME_:
	HOUR_ ':' MINUTE_ (':' SECOND_ ('.' SECOND_FRACTION_)?)?;
fragment HOUR_: [0-2][0-9];
fragment MINUTE_: [0-6][0-9];
fragment SECOND_: [0-6][0-9];
fragment SECOND_FRACTION_:
	[0-9] ([0-9] ([0-9])?)?; // between 1 and 3 digits
